...
class MenuController extends Controller
{
    ...
    {
        // Retrieve all products from the `product` table using the `menus` model
        $menus = menus::all();
    ...
    public function store(Request $request)
{
    ...
    $request->file('image')->move(public_path('asset'), $originalFilename);
    ...
    return redirect()->route('menu.index')->with('success', 'New menu item added!');
}
...
public function destroy($id)
{
    ...
    // Redirect back to the menu page with a success message
    return redirect()->route('menu.index')->with('success', 'Menu item has been deleted (marked as inactive).');
}
